...
var acsAReqMod = angular.module("ACSAReqModule",[]);
acsAReqMod.controller("ACSAReqCtrl",["$scope","AdministerTestsServicePIT2","$rootScope","$window",function($scope,AdministerTestsServicePIT2,$rootScope,$window){
	...
	$scope.acsAReqSubmit = function(operationType,compliance){
		...
		var data = {
				...
				"selectedSuccessorId":$scope.successorCReqTC,
		...
		AdministerTestsServicePIT2.createNewTestCase(data).then(function(response){
			$scope.loading=false;
				...
				if(operationType == 'create')
					$rootScope.successMessage = "ACS AReq Browser test case '"+$scope.testCaseName+"' created";
				...
				$scope.previousTestCase = $scope.selectedTestCase;
					...
					$rootScope.successMessage = "ACS AReq Browser test case '"+$scope.testCaseName+"' deleted";
