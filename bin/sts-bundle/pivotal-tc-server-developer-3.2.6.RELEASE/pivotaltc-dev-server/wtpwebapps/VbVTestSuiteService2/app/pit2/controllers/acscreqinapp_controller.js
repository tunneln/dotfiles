...
var acsCReqMod = angular.module("ACSCReqModuleInApp",[]);
acsCReqMod.controller("ACSCReqCtrlInApp",["$scope","AdministerTestsServiceInappPIT2","$rootScope","$window",function($scope,AdministerTestsServiceInappPIT2,$rootScope,$window){
	...
	$scope.acsCReqSubmit = function(operationType,compliance){
		...
		AdministerTestsServiceInappPIT2.createNewTestCase(data).then(function successCallback(response){
			...
			if(operationType == 'create')
				$rootScope.successMessage = "ACS CReq InApp test case '"+$scope.testCaseName+"' created";
			else if(operationType == 'update'){
				$rootScope.successMessage = "ACS CReq InApp test case '"+$scope.testCaseName+"' updated";
				$scope.previousTestCase = $scope.selectedTestCase;
			}
			else if(operationType == 'delete')
				$rootScope.successMessage = "ACS CReq InApp test case '"+$scope.testCaseName+"' deleted";
