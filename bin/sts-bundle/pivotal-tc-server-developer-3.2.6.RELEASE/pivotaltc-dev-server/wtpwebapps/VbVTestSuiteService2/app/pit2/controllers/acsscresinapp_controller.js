...
var acssCResMod = angular.module("ACSSCResModuleInApp",[]);
acssCResMod.controller("ACSSCResCtrlInApp",["$scope","ACSSCResServiceInApp","$rootScope","$window",function($scope,ACSSCResServiceInApp,$rootScope,$window){
	$scope.msgType = "CRes";
	$scope.testCases = [];
	...
	$scope.sequences = [1,2,3,4,5];
	$scope.previousTestCase;
	/**
	 * Method to get the all the CRes test cases
	 */
	$scope.getTestCases = function(){
		ACSSCResServiceInApp.getTestCases($scope.msgType,$scope.isInApp).then(function(response){
	...
	$scope.setTestCaseData = function(acsstestCase){
		if(acsstestCase != undefined){
			...
			$scope.sequence = acsstestCase.sequence;
			$scope.cResTemplateMsg	= vkbeautify.json(acsstestCase.templateMsg,1);
	...
	$scope.submitTestCase = function(operationType){
		...
		$scope.loading=true;
		ACSSCResServiceInApp.submit(data).then(function successCallback(response){
			$scope.loading=false;

			$rootScope.showErrorAlert = false;	
			$rootScope.showSuccessAlert = true;
			if(operationType == 'create')
				$rootScope.successMessage = "ACSS CRes InApp test case '"+$scope.testCaseName+"' created";
			else if(operationType == 'update'){
				$rootScope.successMessage = "ACSS CRes InApp test case '"+$scope.testCaseName+"' updated";
				$scope.previousTestCase = $scope.selectedTestCase;
			}
			else if(operationType == 'delete')
				$rootScope.successMessage = "ACSS CRes InApp  test case '"+$scope.testCaseName+"' deleted";
			$window.scrollTo(0,0);
