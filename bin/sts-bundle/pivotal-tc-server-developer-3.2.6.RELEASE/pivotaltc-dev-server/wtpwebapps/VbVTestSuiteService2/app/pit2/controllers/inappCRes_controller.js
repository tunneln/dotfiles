...
var pit2runACSTestResCtrlMod = angular.module('PIT2inAppCResCtrlModule', []);

pit2runACSTestResCtrlMod
		.controller(
				'PIT2inAppCResController',
				[
						'$scope',
						'$rootScope',
						'$state',
						...
						function($scope, $rootScope, $state, $window,
								PIT2RunACSTest, $location, PIT2ReviewService,
								ValidateCAVVService,$sce) {
							...
							$scope.cResMessage = vkbeautify.json(
							...
							$scope.jsonFormat = function(type) {
								if (type == "CRes") {
									...
									$scope.cResMessage = vkbeautify.json(aRes,
											1);
								}
								if (type == "expCRes") {
									var expctdcRes = $scope.expectedResponse;
									$scope.expectedResponse = vkbeautify.json(
											expctdcRes, 1);
								}
								if (type == "CReq") {
							...
							$scope.encodeOrDecodeACSHtml = function() {
								...
								PIT2RunACSTest.decodeACSHtml(data).then(function successCallback(responseData){
