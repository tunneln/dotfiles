...
var mpiCResInAppMod = angular.module("MPICResInAppModule",[]);
mpiCResMod.controller("MPICResInAppCtrl",["$scope","AdministerTestsServiceInappPIT2","$rootScope","$window",function($scope,AdministerTestsServiceInappPIT2,$rootScope,$window){
	...
	$scope.mpiCResSubmit = function(operationType,compliance){
		...
		AdministerTestsServiceInappPIT2.createNewTestCase(data).then(function successCallback(response){
			...
			if(operationType == 'create')
				$rootScope.successMessage = "3DS CRes InApp test case '"+$scope.testCaseName+"' created";
			else if(operationType == 'update'){
				$rootScope.successMessage = "3DS CRes InApp test case '"+$scope.testCaseName+"' updated";
				$scope.previousTestCase = $scope.selectedTestCase;
			}
			else if(operationType == 'delete')
				$rootScope.successMessage = "3DS CRes InApp test case '"+$scope.testCaseName+"' deleted";
